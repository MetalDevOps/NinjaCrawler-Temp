...
if (!container) {
  throw new Error('Source sync queue root element was not found.')
}
...
void import('./features/workspace/SourceSyncQueueWindowPage')
  .then(({ SourceSyncQueueWindowPage }) => {
    bootstrapReady = true
    root.render(<SourceSyncQueueWindowPage />)
  })
  .catch((error) => {
...
window.setTimeout(() => {
  if (!bootstrapReady && !bootstrapFailureRendered) {
    renderBootstrapFailure(new Error('Queue status bootstrap timed out while loading UI modules.'))
  }
}, 6000)
