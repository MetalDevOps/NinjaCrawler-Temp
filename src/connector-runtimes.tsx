...
if (!container) {
  throw new Error('Connector runtimes root element was not found.')
}
...
function renderBootstrapFailure(error: unknown) {
  ...
  rootContainer.innerHTML = `
    <div class="runtime-log-bootstrap-failure">
      <h1>Connector Runtimes Failed</h1>
...
void import('./features/connectors/ConnectorRuntimesWindowPage')
  .then(({ ConnectorRuntimesWindowPage }) => {
    bootstrapReady = true
    root.render(<ConnectorRuntimesWindowPage />)
  })
  .catch((error) => {
...
window.setTimeout(() => {
  if (!bootstrapReady && !bootstrapFailureRendered) {
    renderBootstrapFailure(new Error('Connector runtimes bootstrap timed out while loading UI modules.'))
  }
}, 6000)
